...
groupID=${NON_PROD_PLATFORM_DEVELOPER_OBJECT_ID}
...
function kube-test-expected-true() {
  ...
  if [[ $(kubectl auth can-i "${verb}" "${resource}" --subresource="${nonResourceUrl}" -n "${namespace}" --as ${userGroup} --as-group ${groupID}) == "yes" ]]; then
    ...
    printf "${RED}${verb} ${resource} ${nonResourceUrl} = Test Failed Successfully${NC}\n"
    let "totalFailed++"
    printf "Passed: ${totalPassed} / Failed: ${totalFailed}"
...
set-namespace obp-dev
set-user-group AHB-NonProd-AKS-Platform-Developer

heading "pods exec tests"

test-task "A Platform Developer should only be able to create an exec into pods"
echo
...
test-task "A Platform Developer should only be able to create portforwards into pods"
echo
...
test-task "A Platform Developer should only be able to create portforwards into services"
echo
...
test-task "A Platform Developer should only be able to create selfsubjectaccessreviews"
echo
...
test-task "A Platform Developer should only be able to create selfsubjectrulesreviews"
echo
...
test-task "A Platform Developer should be able to get, list, watch and delete pods"
echo

kube-test-expected-true get pods
kube-test-expected-true list pods
kube-test-expected-true watch pods
kube-test-expected-true create pods
kube-test-expected-false update pods
kube-test-expected-false patch pods
kube-test-expected-true delete pods

heading "configmaps tests"

test-task "A Platform Developer should be able to get, list and watch pods"
echo
...
test-task "A Platform Developer should be able to get, list and watch deployments"
echo
...
test-task "A Platform Developer should be able to get, list and watch events"
echo

kube-test-expected-true get events
kube-test-expected-true list events
kube-test-expected-true watch events
...
test-task "A Platform Developer should be able to get, list and watch namespaces"
echo
...
test-task "A Platform Developer should be able to get, list and watch pod logs"
echo
...
test-task "A Platform Developer should be able to get, list and watch services"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /api/*"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /apis/*"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /healthz"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /livez"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /openapi/*"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /readyz"
echo
...
test-task "A Platform Developer should only be able to get the endpoint /version/"
echo
...
kubectl auth can-i --list --namespace=${namespace} --as AHB-NonProd-AKS-Platform-Developer --as-group ${groupID}
echo
echo
