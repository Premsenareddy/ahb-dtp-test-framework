...
@Tag("Cards")
public class GetSpendingLimitsTests {
    ...
    private void existingUserLogin() {
        //disabling until updated for cert login
        envUtils.shouldSkipHps(Environments.NONE);
    ...
    @Order(1)
    @Test()
    public void get_spending_limit_for_user() {
        //AHBDB-12951: Test failing - fixed by updating dev user

        setupTestUser();
        TEST("AHBDB-3297 View cards spending limit");
        ...
        final ReadCardLimits1 cardLimits = cardsApiFlows.fetchCardLimitsForTransactionType(alphaTestUser, TransactionType.PURCHASE.getLabel(), cardId);
        Assertions.assertEquals(cardLimits.getData().getLimits().getLimitTypeWording(),TransactionType.PURCHASE.getLabel().toUpperCase());
    ...
    @Order(2)
    @Test()
    public void negative_get_spending_limit_for_another_user() {
        TEST("AHBDB-12951: Test failing");
        ...
        AND("A valid cardId is created using the unmasked digits of the other users card number : " + cardId);

        THEN("The get service will return a 404");
        this.cardsApiFlows.fetchCardLimitsForTransactionTypeError(alphaTestUser, TransactionType.PURCHASE.getLabel(), cardId, 404);
    ...
    public void negative_test_get_spending_limit_invalid_cardNo_length(String InvalidCardNoLength) {
        setupTestUser();
        TEST("AHBDB-3297 View cards spending limit with an invalid card length : " + InvalidCardNoLength);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a cardNo of invalid length : " + InvalidCardNoLength);

        THEN("The get service will return a 400");
        final OBErrorResponse1  cardLimits = cardsApiFlows.fetchCardLimitsForTransactionTypeError(alphaTestUser, TransactionType.PURCHASE.getLabel(), InvalidCardNoLength, 400);
    ...
    public void negative_test_get_spending_limit_invalid_cardNo_content(String InvalidCardNoContent) {
        setupTestUser();
        TEST("AHBDB-3297 View cards spending limit with an invalid card content : " + InvalidCardNoContent);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a cardNo of invalid content : " + InvalidCardNoContent);

        THEN("The get service will return a 400");
        final OBErrorResponse1  cardLimits = cardsApiFlows.fetchCardLimitsForTransactionTypeError(alphaTestUser, TransactionType.PURCHASE.getLabel(), InvalidCardNoContent, 400);
    ...
    @Order(4)
    @Test
    public void negative_test_get_spending_limit_invalid_cardID_not_found() {
        setupTestUser();
        String InvalidCardIDNotFound = RandomDataGenerator.generateRandomNumeric(10);
        TEST("AHBDB-3297 View cards spending limit with a not found cardNo : " + InvalidCardIDNotFound);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a not found cardNo : " + InvalidCardIDNotFound);

        THEN("The get service will return a 404");
        final OBErrorResponse1  cardLimits = cardsApiFlows.fetchCardLimitsForTransactionTypeError(alphaTestUser, TransactionType.PURCHASE.getLabel(), InvalidCardIDNotFound, 404);
    ...
    @Order(6)
    @Test
    public void negative_test_get_spending_limit_invalid_token() {
        setupTestUser();
        TEST("AHBDB-3297 View cards spending limit for a valid user with an invalid token returns 401");
        GIVEN("I have a valid customer with accounts scope");
        ...
        this.cardsApiFlows.fetchCardLimitsForTransactionTypeErrorVoid(alphaTestUser, TransactionType.PURCHASE.getLabel(), cardId, 401);
        DONE();
    }
