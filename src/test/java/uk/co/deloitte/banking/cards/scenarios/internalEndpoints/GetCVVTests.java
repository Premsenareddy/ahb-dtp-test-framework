...
public class GetCVVTests {
    ...
    public void negative_test_get_cardCVV_invalid_cardNo_length(String InvalidCardNoLength) {
        setupTestUser();
        TEST("AHBDB-3414 get debit card cvv for a valid user with an invalid card length : " + InvalidCardNoLength);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a cardNo of invalid length : " + InvalidCardNoLength);

        THEN("The get cvv service will return a 400");
        final OBErrorResponse1 cardCVvDetailsError = this.cardsApiFlows.fetchCardsCvvForUserError(alphaTestUser, InvalidCardNoLength, 400);

        Assertions.assertTrue(cardCVvDetailsError.getMessage().contains(CARD_LENGTH_ERROR), "Error message was not as expected, " +
    ...
    public void negative_test_get_cardCVV_invalid_cardNo_content(String InvalidCardNoContent) {
        setupTestUser();
        TEST("AHBDB-3414 get debit card cvv for a valid user with an invalid card content : " + InvalidCardNoContent);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a cardNo of invalid content : " + InvalidCardNoContent);

        THEN("The get cvv service will return a 400");
        final OBErrorResponse1 cardCVvDetailsError = this.cardsApiFlows.fetchCardsCvvForUserError(alphaTestUser, InvalidCardNoContent, 400);

        Assertions.assertTrue(cardCVvDetailsError.getMessage().contains(CARD_REGEX_ERROR), "Error message was not as expected, " +
    ...
    @Order(4)
    @Test
    public void negative_test_get_cardCVV_invalid_cardNo_not_found() {
        setupTestUser();
        String InvalidCardIDNotFound = RandomDataGenerator.generateRandomNumeric(10);
        TEST("AHBDB-3414 get debit card cvv for a valid user with a not found cardNo : " + InvalidCardIDNotFound);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a not found cardNo : " + InvalidCardIDNotFound);

        THEN("The get cvv service will return a 404");
        final OBErrorResponse1 cardCVvDetailsError = this.cardsApiFlows.fetchCardsCvvForUserError(alphaTestUser, InvalidCardIDNotFound, 404);
        Assertions.assertTrue(cardCVvDetailsError.getMessage().contains(CARD_NOT_FOUND_ERROR), "Error message was not as expected, " +
    ...
    @Order(4)
    @Test
    public void negative_test_get_cardCVV_another_users_card() {
        setupTestUser();
        ...
        TEST("AHBDB-3414 get debit card cvv for a valid user with another users card : " + cardId);
        GIVEN("I have a valid customer with accounts scope");

        WHEN("The user has a not found cardNo : " + cardId);

        THEN("The get cvv service will return a 404");
        final OBErrorResponse1 cardCVvDetailsError = this.cardsApiFlows.fetchCardsCvvForUserError(alphaTestUser, cardId, 404);
        Assertions.assertTrue(cardCVvDetailsError.getMessage().contains(CARD_NOT_FOUND_ERROR), "Error message was not as expected, " +
                "test expected : " + CARD_NOT_FOUND_ERROR);
        DONE();
    }
    ...
    @Order(6)
    @Test
    public void negative_test_get_cardCVV_invalid_token() {
        setupTestUser();
        TEST("AHBDB-3414 get debit card cvv for a valid user with an invalid token returns 401");
        GIVEN("I have a valid customer with accounts scope");
        ...
        this.cardsApiFlows.fetchCardsCvvForUserErrorVoid(alphaTestUser, cardId, 401);
        DONE();
    }
